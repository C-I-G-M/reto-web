...
const {FechaSolicitud, ID_Establecimiento, ID_DirectorTecnico, ID_Propietario,TipoDeSolicitud,ReciboPagoTasasNumero,EstadoSolicitud, FirmadoPorInspector,FirmaDirectorTecnico,FIOC,F004Presentado,LODT} = req.body;
 const userInfo = getUserInfo(req.user);
const ID_Usuario = userInfo.id; // extraído del token


   if (!!!FechaSolicitud || !!!ID_Establecimiento || !!!ID_DirectorTecnico || !!!ID_Propietario || !!!TipoDeSolicitud || !!!ReciboPagoTasasNumero || !!!EstadoSolicitud || !!!FirmadoPorInspector|| !!!FirmaDirectorTecnico || !!! FIOC || !!!F004Presentado || !!! LODT) {
        return res.status(400).json(jsonResponse(400,{error:"Solicitud Incompleta (todos los campos son obligatorios) "}));
    }
//editar condiciones por variables actuales
